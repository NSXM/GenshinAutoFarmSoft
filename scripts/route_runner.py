...
class RouteRunner:
    ...
    @staticmethod
    def _circ_median(vals_deg):
        """Круговая медиана списка углов (та же идея, что в minimap_reader._median_arrow):
        элемент списка с минимальной суммой |угловых разниц| до остальных — устойчив
        к редким выбросам/спайкам стрелки, в отличие от обычного среднего."""
        if not vals_deg:
            return None
        best, best_s = vals_deg[0], 1e18
        for x in vals_deg:
            s = sum(abs(angle_diff_deg(x, y)) for y in vals_deg)
            if s < best_s:
                best_s, best = s, x
        return best

    def _robust_heading(self, secs: float, require_moving: bool = False,
                         outlier_deg: float = 25.0):
        """
        Устойчивое к выбросам усреднение курса для СТАРТОВОЙ калибровки смещения.
        В отличие от _avg_heading (простое круговое среднее — один редкий спайк
        стрелки уводит результат), здесь:
          1) копим сырые отсчёты secs секунд,
          2) находим круговую медиану (устойчивый "центр"),
          3) отбрасываем отсчёты дальше outlier_deg от медианы (спайки/глюк детектора),
          4) считаем круговое среднее по оставшимся — это и есть финальный курс.
        Возвращает (heading_deg, spread_deg, n_used, n_total) — spread = круговая
        медиана |отклонений| от итогового курса (оценка качества калибровки, печатается
        в лог, чтобы разброс между прогонами был виден, а не скрыт).
        Возвращает (None, None, 0, 0), если не набралось достаточно отсчётов.
        """
        raw = []
        t_end = time.monotonic() + secs
        while time.monotonic() < t_end:
            ...
            if require_moving and not p.moving:
                continue
            raw.append(p.heading_deg)
        n_total = len(raw)
        if n_total < 3:
            return None, None, 0, n_total
        med = self._circ_median(raw)
        kept = [a for a in raw if abs(angle_diff_deg(a, med)) <= outlier_deg]
        if len(kept) < 3:
            kept = raw   # выбросов оказалось подозрительно много — не фильтруем вслепую
        s = sum(math.sin(math.radians(a)) for a in kept)
        c = sum(math.cos(math.radians(a)) for a in kept)
        heading = (math.degrees(math.atan2(s, c)) + 360.0) % 360.0
        spread = self._circ_median([abs(angle_diff_deg(a, heading)) for a in kept])
        return heading, spread, len(kept), n_total
    ...
    def run(self):
        ...
        #
        # ВАЖНО: это ЕДИНСТВЕННОЕ измерение за весь прогон — ошибка здесь не лечится
        # потом ничем и тянется на всю дистанцию (видна как дрифт на длинных прямых,
        # разный по знаку от прогона к прогону). Раньше брали 0.8с простым круговым
        # средним — один редкий спайк стрелки внутри этого окна уводил _hdg_offset на
        # несколько градусов незаметно для нас. Теперь: дольше (2.0с) + устойчивая к
        # выбросам медиана + печатаем разброс (spread), чтобы кривая калибровка была
        # ВИДНА в логе, а не пряталась за одной "красивой" цифрой смещения.
        self._hdg_offset = 0.0
        brg0 = self._start_bearing()
        hs, spread, n_used, n_total = self._robust_heading(2.0)
        if hs is not None and brg0 is not None:
            self._hdg_offset = angle_diff_deg(hs, brg0)
            print(f"[run] калибровка курса: курс стоя {hs:.0f}° (разброс ~{spread:.1f}°, "
                  f"{n_used}/{n_total} отсчётов), пеленг на цель {brg0:.0f}° "
                  f"→ смещение {self._hdg_offset:+.0f}° (предполагаю старт ЛИЦОМ вдоль маршрута!)")
            if spread is not None and spread > 8.0:
                print(f"[run] ВНИМАНИЕ: разброс курса при калибровке большой (~{spread:.1f}°) — "
                      f"стрелка нестабильна/мерцает, смещение может быть неточным. Если бот "
                      f"будет дрейфовать, попробуй перезапустить, постояв чуть дольше/спокойнее.")
        else:
        ...
        try:
            while self._running:
                ...
                if self.hud is not None:
                    cmd = self.hud.update(self.tracker.last_frame,
                                          (self._dr_pos[0], self._dr_pos[1]),
