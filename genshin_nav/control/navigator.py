...
class Navigator:
    def __init__(self, cfg, route: List[Waypoint]):
        ...
        # --- регулятор курса (доворот на точку) ---
        # Раньше было bang-bang: при |err|>12° крутили камеру на весь err
        # (до 25°). Любая неточность исполнения/измерения курса даёт перелёт,
        # на следующем кадре err меняет знак -> рысканье влево-вправо.
        # Лечим: мёртвая зона + пропорциональный шаг (доля от ошибки), плюс
        # явный приоритет состояний AVOID > ALIGN > WOBBLE, чтобы повороты
        # из разных систем не складывались в одном кадре.
        self.heading_deadband_deg = getattr(cfg.control, "heading_deadband_deg", 4.0)
        self.heading_kp = getattr(cfg.control, "heading_kp", 0.5)
        self.heading_max_step_deg = getattr(cfg.control, "heading_max_step_deg", 15.0)
    ...
    def run(self):
        ...
        try:
            while self._running and self.wp_idx < len(self.route):
                ...
                # --- доворот на цель / обход препятствий / wobble ---
                # ВАЖНО: раньше доворот (ALIGN) и обход препятствия (AVOID)
                # могли оба вызвать rotate_camera() в ОДНОМ кадре — повороты
                # складывались в случайную сторону. Плюс доворот был bang-bang
                # (весь err разом) -> перелёт -> err меняет знак -> рысканье.
                # Теперь: явный приоритет AVOID > ALIGN > WOBBLE, и доворот —
                # пропорциональный шаг с мёртвой зоной, а не весь err сразу.
                target = self._bearing_to_wp(st.player_xy)
                err = angle_diff_deg(target, st.heading_deg) if target is not None else 0.0
                aligning = abs(err) > self.heading_deadband_deg
                ...
                if obs is not None:
                    # препятствие важнее доворота на маршрут — доворот этим
                    # кадром пропускаем, чтобы не складывать два поворота
                    lateral, fwd = obs
                    self.state = State.AVOID
                    self.inp.rotate_camera(-18.0 if lateral > 0 else 18.0)
                    if fwd < 1.5:               # совсем близко — прыжок через кочку
                        self.inp.jump()
                    self.inp.start_moving()
                elif aligning:
                    # пропорциональный доворот: только доля от ошибки за тик,
                    # с ограничением по модулю. err сходится к 0 плавно, без
                    # перелётов на следующих кадрах. Wobble в это время не
                    # делаем — он сам по себе шумит курсом, а нам сейчас нужно
                    # сходиться, а не добавлять возмущение.
                    step = max(-self.heading_max_step_deg,
                               min(self.heading_max_step_deg, err * self.heading_kp))
                    self.inp.rotate_camera(step)
                    self.state = State.ALIGN
                    self.inp.start_moving()
                else:
                    # курс в норме, препятствий нет — можно безопасно качнуть
                    # камеру для триангуляции (это и есть SAM-кейфрейм)
                    if frame_i % 20 == 0:
