...
class RouteRunner:
    ...
    def run(self):
        ...
        route_sh = getattr(self.route, "start_heading", None)
        if route_sh is not None and brg0 is not None:
            # НАДЁЖНЫЙ путь: смещение берём из курса, записанного В НАЧАЛЕ ДВИЖЕНИЯ
            # по маршруту (record_route.start_heading). Оно не зависит от того, как
            # ты встал при запуске — руление само довернёт персонажа вдоль маршрута
            # (стартовый err = насколько твоё текущее лицо отличается от записанного).
            self._hdg_offset = angle_diff_deg(route_sh, brg0)
            print(f"[run] калибровка курса ИЗ МАРШРУТА: start_heading {route_sh:.0f}°, "
                  f"пеленг на цель {brg0:.0f}° → смещение {self._hdg_offset:+.0f}°. "
                  f"Стартовая поза НЕ важна — бот довернётся сам.")
        else:
            # ФОЛБЭК (старый маршрут без start_heading): предполагаем старт лицом
            # вдоль маршрута и меряем курс стоя. Перезапиши маршрут — будет надёжнее.
            hs = self._avg_heading(0.8)
            if hs is not None and brg0 is not None:
                self._hdg_offset = angle_diff_deg(hs, brg0)
                print(f"[run] калибровка курса (фолбэк): курс стоя {hs:.0f}°, пеленг {brg0:.0f}° "
                      f"→ смещение {self._hdg_offset:+.0f}° (СТАНЬ ЛИЦОМ вдоль маршрута! "
                      f"перезапиши маршрут — появится start_heading и поза станет не важна)")
            else:
        ...
        try:
            while self._running:
                ...
                # ПОЗИЦИЯ для follower. По умолчанию — ОДОМЕТРИЯ миникарты
                # (pose.player_xy), та же, в которой ПИСАЛСЯ маршрут → одна система
                # координат, без зеркала/поворота между записью и прогоном. dead_reckon
                # оставлен опцией (config control.position_source).
                if getattr(self.cfg.control, "position_source", "odometry") == "dead_reckon":
                ...
                trace.write(f"{t0 - t_start:.3f},{pos[0]:.2f},"
                            f"{pos[1]:.2f},{hdg:.1f},"
