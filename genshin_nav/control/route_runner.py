...
class RouteRunner:
    def __init__(self, cfg, route):
        ...
        self._flips = 0
        self._sign_locked = False        # знак установлен пробой -> auto-flip отключён
    ...
    def _pivot(self, err_deg: float):
        """Крупный доворот СТОЯ: останавливаемся, крутим камеру мышью (open-loop,
        стоя она слушается полностью), едем дальше. Доворачиваем на 0.8·err —
        чуть недокручиваем, чтобы не было перелёта/осцилляций; остаток доберёт
        мягкое руление на ходу. Знак ошибётся — поймает auto-flip на след. кадрах."""
        ...
        time.sleep(0.30)                # дать камере довернуться
        self.inp.start_moving()         # снова едем; курс пере-замерится на след. кадрах

    def _steer(self, err_deg: float, now: float):
        if self.units_per_deg is None or abs(err_deg) < self.deadzone:
            return
        if now - self._last_steer_t < self.throttle:    # не чаще лага курса
            return
        self._last_steer_t = now
        dx = err_deg * self.gain * self.units_per_deg
    ...
    def run(self):
        ...
        self.inp.start_moving()         # сразу едем, без калибровочного угона
        ...
        try:
            while self._running:
                ...
                if pose is None:
                    time.sleep(0.001)
                    continue
                d = self.follower.step(pose.player_xy, pose.heading_deg)
                if d.done:
                ...
                # Рулим/оцениваем знак ТОЛЬКО при move=Y: тогда курс из сдвига карты
                # надёжен. move=n -> курс с конуса мусорит, рулить по нему нельзя.
                err = d.heading_err_deg
                if pose.moving:
                    self._auto_sign(err, t0)
                    if abs(err) > self.align_big_deg:
                        self._pivot(err)           # крупный доворот стоя, затем едем
                    else:
                        self._steer(err, t0)       # мелкая правка на ходу
                else:
                    self._diverge_since = None     # не копим разнос по мусорному курсу
