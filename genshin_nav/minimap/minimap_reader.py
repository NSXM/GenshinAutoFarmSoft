...
class MinimapReader:
    def __init__(self, cfg, atlas: Optional[np.ndarray] = None):
        ...
        self._tracker = None      # MinimapTracker, лениво — только если heading_source=="tracker"
    ...
    def read_heading(self, minimap_bgr: np.ndarray) -> Optional[float]:
        ...
        heading = (np.degrees(np.arctan2(-dx, dy)) + 180.0 + 360.0) % 360.0 # инвертировано: нос и хвост поменяны
    ...
    def _tracker_heading(self, frame_bgr: np.ndarray) -> Optional[float]:
        """Курс от внешнего MinimapTracker (+поправка конвенции). None, если
        маркер не найден — тогда read() остаётся на arrow/map/cone каскаде."""
        if self._tracker is None:
            from .marker_tracker import MinimapTracker
            self._tracker = MinimapTracker.from_cfg(self.cfg)
        angle, _conf = self._tracker.detect(frame_bgr)
        if angle is None:
            return None
        off = getattr(self.cfg, "tracker_heading_offset_deg", 0.0)
        return float((angle + off) % 360.0)
    ...
    def read(self, frame_bgr: np.ndarray) -> MinimapReading:
        ...
        # АЛЬТЕРНАТИВНЫЙ источник: внешний MinimapTracker (ось+конус). По умолчанию
        # heading_source=="arrow" — эта ветка не трогается. На "tracker" заменяем
        # курс его оценкой (с поправкой конвенции), оставляя arrow/map/cone фолбэком.
        if getattr(self.cfg, "heading_source", "arrow") == "tracker":
            trk_h = self._tracker_heading(frame_bgr)
            if trk_h is not None:
                heading = trk_h
